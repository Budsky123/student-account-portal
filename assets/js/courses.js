...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const storeState = window.CourseStore
    ? CourseStore.getState()
    : fallbackState;
  let enrolled = [...storeState.enrolled];
  let available = [...storeState.available];
  const MAX_UNITS = storeState.maxUnits || fallbackState.maxUnits;
  ...
  const syncState = () => {
    if (window.CourseStore) {
      CourseStore.saveState({ maxUnits: MAX_UNITS, enrolled, available });
    }
  };
  ...
  const getTotalUnits = () =>
    enrolled.reduce((sum, course) => sum + course.units, 0);
  ...
  const renderLists = () => {
    ...
    availableList.innerHTML = toRender
      .map((c) => {
  ...
  const showToast = (message, type, withUndo = false) => {
    ...
    const undoHtml = withUndo
      ? `<button class="toast-undo-btn" onclick="window.undoDrop()">Undo</button>`
      : "";
